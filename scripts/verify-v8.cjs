...
(async()=>{
 ...
 await page.goto(base+'/');assert.match(await page.locator('h1').innerText(),/Stop putting your/);assert.equal(await page.locator('meta[name="p44-content-version"]').getAttribute('content'),'8.0');
 ...
 await page.emulateMedia({reducedMotion:'reduce'});await start();await page.locator('#back').click();assert.equal(await page.locator('.q8-signal-intro .signal-wave').evaluate(e=>getComputedStyle(e).animationName),'none');
