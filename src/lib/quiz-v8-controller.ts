...
function stopScreenMotion(){gsap.killTweensOf(root);gsap.set(root,{clearProps:'opacity,visibility,transform'});}
...
function render(){
  ...
  if(id==='screen_0'){
    ...
    root.innerHTML=`<div class="q8-intro-copy"><p class="section-kicker">${t(content.editorial.quizKicker)}</p><h1 tabindex="-1">${t(s.title)}</h1>${s.body.map(p=>`<p class="lead">${t(p)}</p>`).join('')}<button class="button button-primary" id="start-quiz">${t(s.cta)} ${arrow}</button><p class="button-note">${t(s.note)}</p></div><div class="q8-signal-intro"><figure class="quiz-life-photo"><img class="signal-portrait" src="/brand/next-step-canada-768.webp" srcset="/brand/next-step-canada-480.webp 480w, /brand/next-step-canada-768.webp 768w" sizes="(max-width: 760px) calc(100vw - 40px), 42vw" width="768" height="512" fetchpriority="high" alt="${t(content.editorial.photoAlt)}" /><figcaption>${t(content.editorial.illustrationLabel)}</figcaption></figure><div class="quiz-reflection-preview"><p class="preview-heading">${t(content.editorial.quizPreviewLabel)}</p><h2>${t(content.editorial.quizPreviewTitle)}</h2><p>${t(content.editorial.quizPreviewBody)}</p>${signalArt(0,true)}<p class="signal-caption"><span>${t(ui.signalStart)}</span><span>${t(ui.signalEnd)}</span></p></div><button class="text-button" id="skip-intro">${t(ui.skipIntro)}</button></div>`;
