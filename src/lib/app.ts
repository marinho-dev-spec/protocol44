...
/** Troca de tela com transição de sintonização (blur → nítido). */
function swap(html: string, onMount?: () => void) {
  const mount = () => {
    ...
    if (vivo()) {
      ...
      const itens = app.querySelectorAll('.st');
      if (itens.length) {
        gsap.fromTo(
          itens,
          { opacity: 0, y: 8 },
      ...
      gsap.set(app, { clearProps: 'opacity,filter,transform' });
    }
  };
  if (vivo()) {
    let montado = false;
    ...
    gsap.to(app, { opacity: 0, filter: 'blur(8px)', y: -6, duration: 0.22, ease: 'power1.in', overwrite: true, onComplete: montarUmaVez });
    // fallback: se a aba ocultar no meio da transição, o rAF congela e o onComplete não dispara
    setTimeout(montarUmaVez, 450);
  } else {
...
const SHELL_QUIZ = 'w-full max-w-md mx-auto px-5 pt-20 pb-16 min-h-dvh flex flex-col justify-center';
const SHELL_RESULTADO = 'w-full max-w-xl mx-auto px-5 pt-24 pb-20';
...
function prumoSvg(id: string, cls = '') {
  ...
  <svg class="mx-auto h-full ${cls}" viewBox="0 0 160 96" aria-hidden="true">
    <line id="${id}-rest" x1="18" y1="80" x2="142" y2="80" stroke="#443d36" stroke-width="1.5"/>
    <path d="M 46 76 A 42 42 0 0 1 114 76" fill="none" stroke="#8fa5b9" stroke-width="1" stroke-dasharray="2 5" opacity="0.4"/>
    <line x1="66" y1="10" x2="94" y2="10" stroke="#a39b90" stroke-width="2.2" stroke-linecap="round"/>
    <g id="${id}-arm">
      <line x1="80" y1="10" x2="80" y2="58" stroke="#a39b90" stroke-width="1.4"/>
      <path id="${id}-bob" d="M 80 57 L 87 65 L 80 86 L 73 65 Z" fill="#8fa5b9"/>
    </g>
  </svg>`;
}
...
function telaZero() {
  ...
  swap(
    ...
    <div class="text-center">
      <div class="st flex justify-center">
        <img src="/logo-mark.svg" alt="" width="58" height="58" class="rounded-2xl shadow-[0_14px_40px_rgba(0,0,0,0.5)]" id="marca0" />
      </div>
      <p class="st eyebrow mt-6" id="medindo">MEDINDO<span id="dots">...</span></p>
      <div class="st mt-7 h-28">${prumoSvg('t0')}</div>
      <div class="st painel text-left p-5 mt-7">
        <p class="text-ash text-[0.95rem] leading-relaxed">${D.TELA0.texto}</p>
      </div>
      <button class="st cta mt-7 w-full" id="btn-start">${D.TELA0.cta}</button>
    </div>
  `,
    () => {
      document.getElementById('btn-start')!.addEventListener('click', avancar);
      if (vivo()) {
        // o pendulo procurando: oscila devagar e nunca assenta nesta tela
        gsap.fromTo('#t0-arm', { rotate: 13 }, {
          rotate: -13, duration: 2.4, repeat: -1, yoyo: true,
          ease: 'sine.inOut', svgOrigin: '80 10',
        });
        gsap.to('#dots', { opacity: 0.2, duration: 0.6, repeat: -1, yoyo: true });
        gsap.fromTo('#marca0', { x: 9, opacity: 0 }, { x: 0, opacity: 1, duration: 0.7, ease: 'power3.out' });
      }
    }
  );
}
...
function bufferP9() {
  ...
  setTimeout(avancar, RM ? 300 : 1700);
}
...
function cardReforco(inner: string, ctaLabel: string, extra = '') {
  return `
    <div class="st mb-5 h-14 opacity-45">${prumoSvg('rf' + (++_prumoN))}</div>
  ...
  if (tipo === 'crente') {
    ...
    `,
      () => {
        ...
        setTimeout(montarChecagem, RM ? 200 : 6000);
      }
    );
  } else if (tipo === 'duvida') {
    swap(
      ...
      () => {
        ...
        document.querySelectorAll<HTMLButtonElement>('[data-palavra]').forEach((b) =>
          b.addEventListener('click', () => {
            ...
            setTimeout(montarChecagem, RM ? 200 : 1400);
          })
        );
      }
    );
...
function gateEmail() {
  swap(
    ...
    <form id="form-email" class="st mt-6">
      <input type="email" required id="inp-email" inputmode="email" autocomplete="email" placeholder="${D.GATE.emailPlaceholder}"
        class="w-full bg-ridge border border-seam-hi rounded-xl px-4 py-3.5 text-chalk placeholder:text-dust text-[0.95rem]" />
      <p id="err-email" class="hidden text-clay text-[0.8rem] mt-2">Confere esse e-mail? Parece faltar um pedaço.</p>
...
function gateNome() {
  swap(
    ...
    <form id="form-nome" class="st mt-6">
      <input type="text" required id="inp-nome" autocomplete="given-name" placeholder="${D.GATE.nomePlaceholder}"
        class="w-full bg-ridge border border-seam-hi rounded-xl px-4 py-3.5 text-chalk placeholder:text-dust text-[0.95rem]" />
...
function sec(inner: string, cls = '') {
  return `<section class="sec opacity-0 ${cls}">${inner}</section>`;
}

function diagramaHtml(v: D.Variacao) {
  const painel = (titulo: string, comRuido: boolean, de: string, bloco: string, para: string) => `
    <div class="painel p-4 rounded-xl!">
      <p class="eyebrow text-[0.6rem]! mb-3">${titulo}</p>
      <div class="flex items-center gap-2 text-[0.72rem] leading-tight">
        <span class="shrink-0 max-w-[30%]">${de}</span>
        <svg class="grow h-8" viewBox="0 0 100 24" preserveAspectRatio="none" aria-hidden="true">
          ${comRuido
            ? `<line x1="0" y1="19" x2="100" y2="19" stroke="#8fa5b9" stroke-width="1.6"/>
               <path d="M 36 19 A 15 15 0 0 1 64 19" fill="none" stroke="#a39b90" stroke-width="0.8" stroke-dasharray="2 3"/>
               <path d="M 64 13 L 67 16 L 64 23 L 61 16 Z" fill="#8fa5b9"/>`
            : `<line x1="0" y1="7" x2="100" y2="7" stroke="#d9a441" stroke-width="1.8"/>
               <path d="M 50 1 L 53 4 L 50 11 L 47 4 Z" fill="#d9a441"/>`}
        </svg>
        <span class="shrink-0 max-w-[34%] text-right">${para}</span>
      </div>
      ${comRuido ? `<p class="font-mono text-[0.62rem] text-ash mt-2 text-center">${bloco}</p>` : `<p class="font-mono text-[0.62rem] text-brass mt-2 text-center">${bloco}</p>`}
    </div>`;
  return `
    ...
    <p class="font-mono text-[0.62rem] text-dust mt-2 text-center">${D.DIAGRAMA_LEGENDA}</p>`;
}
  ...
  app.innerHTML = `
    ${sec(`
      <div class="flex items-center gap-3 mb-5">
        <span class="eyebrow">Diagnóstico · Sinal Bloqueado</span>
        <span class="h-px flex-1 bg-seam"></span>
      </div>
    ...
    ${sec(`
      <div class="painel border-tide/60! relative overflow-hidden p-7 mt-2">
        ...
        <p class="eyebrow text-tide! mb-3">Seu padrão</p>
    ...
    ${sec(`<p class="eyebrow mb-3">O mecanismo, desenhado</p>${diagramaHtml(v)}`, 'mt-10')}
    ...
    <div class="mt-14 border-t border-seam pt-10" id="fecho">
      ...
      <div class="sim hidden" data-sim="4">
        ...
        <a href="/checkout" class="cta cta-verde w-full mt-7 no-underline" id="cta-final">${F.cta}</a>
    ...
    <footer class="mt-16 pt-5 border-t border-seam">
      <p class="font-mono text-[0.65rem] text-dust leading-relaxed">Protocol 44 · The Setpoint Protocol — protótipo interno. Baseado na análise de um corpus de relatos públicos (contagem em reconciliação, ver INDICE_GERAL); nada nesta página promete resultado individual. Não é terapia nem tratamento médico.</p>
    </footer>
  `;
...
function montarResultado() {
  // reveals por scroll — sintonização por seção (aba oculta/RM: tudo visível de imediato)
  if (vivo()) {
    document.querySelectorAll<HTMLElement>('.sec').forEach((s) => {
      gsap.fromTo(
        s,
  ...
  // A contagem animada do 97,87% foi removida em 07/09: o numero nao tem
  // lastro (ver INDICE_GERAL.md, achado 1). Volta quando a analise do corpus
  // devolver o percentual real com criterio escrito.

  // o momento-assinatura: a onda alisa no payoff ("Foi a sintonia.")
  const payoff = document.getElementById('payoff-wave');
  const assentar = () => {
    // o momento-assinatura da marca: o peso para no eixo e a linha de repouso
    // acende em latao. E o unico uso de brilho permitido no funil.
    gsap.fromTo('#pw-arm', { rotate: 17 }, {
      rotate: 0, duration: 2.6, ease: 'elastic.out(1, 0.42)', svgOrigin: '80 10',
    });
    gsap.to('#pw-bob', { fill: '#d9a441', duration: 1.2, delay: 0.9 });
    gsap.to('#pw-rest', { stroke: '#d9a441', strokeWidth: 2, duration: 1.2, delay: 0.9 });
  };
  if (payoff && !vivo()) {
    gsap.set('#pw-bob', { fill: '#d9a441' });
    gsap.set('#pw-rest', { stroke: '#d9a441', strokeWidth: 2 });
  }
  if (payoff && vivo()) {
    ScrollTrigger.create({ trigger: payoff, start: 'top 78%', once: true, onEnter: assentar });
  }
