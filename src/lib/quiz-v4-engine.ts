...
import { PERGUNTAS, type Opcao, type Persona, type Vilao } from './quiz-v4-copy';
...
const CHAVE = 'protocol44-quiz-v4';
...
export function restaurar(): { estado: Estado; passo: number } | null {
  try {
    ...
    if (!dados || !dados.estado || !Number.isInteger(dados.passo) || dados.passo < 0 || dados.passo > 18) return null;
    const limpo: Estado = { respostas: {} };
    for (const [pid, q] of Object.entries(PERGUNTAS)) {
      const rawIds = dados.estado.respostas?.[pid];
      if (!Array.isArray(rawIds)) continue;
      let ids = q.opcoes.filter(o => rawIds.includes(o.id)).map(o => o.id);
      const exclusiva = q.opcoes.find(o => o.exclusiva && ids.includes(o.id));
      if (exclusiva) ids = [exclusiva.id];
      if (!q.multipla) ids = ids.slice(0, 1);
      if (ids.length) limpo.respostas[pid] = ids;
    }
    if (typeof dados.estado.nome === 'string') limpo.nome = dados.estado.nome.slice(0,80);
    if (typeof dados.estado.email === 'string') limpo.email = dados.estado.email.slice(0,254);
    const priming = dados.estado.priming;
    if (priming && ['crente','duvida','cetica'].includes(priming.tipo)) {
      limpo.priming = { tipo: priming.tipo };
      if (['Sim','Mais ou menos','Não'].includes(priming.checagem)) limpo.priming.checagem = priming.checagem;
      if (['Dinheiro','Medo','Cansaço','Vazio'].includes(priming.palavra)) limpo.priming.palavra = priming.palavra;
    }
    const passos = [1,2,3,4,6,7,9,10,11,14];
    const faltante = Object.keys(PERGUNTAS).findIndex(pid => !limpo.respostas[pid]?.length);
    const maxPasso = faltante >= 0 ? passos[faltante] : !limpo.email ? 16 : !limpo.nome ? 17 : 18;
    return { estado: limpo, passo: Math.min(dados.passo, maxPasso) };
  } catch {}
  return null;
}
